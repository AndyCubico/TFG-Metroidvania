...
public partial struct PathfindingJob : IJobEntity
{
    ...
    public void Execute(in pathfinding.PathfindingParamsComponent pathfindingParams, ref pathfinding.PathIndexComponent pathIndexComponent, ref DynamicBuffer<pathfinding.PathPosition> pathPositionBuffer)
    {
        // Set up grid
        for (int i = 0; i < pathNodeArray.Length; i++)
        {
            PathNode pathNode = pathNodeArray[i];
            pathNode.hCost = CalculateDistanceCost(new int2(pathNode.x, pathNode.y), pathfindingParams.endingPosition);
        ...
        int endNodeIndex = CalculateIndex(pathfindingParams.endingPosition.x, pathfindingParams.endingPosition.y, gridSize.x);

        // Start
        PathNode startNode = pathNodeArray[CalculateIndex(pathfindingParams.startingPosition.x, pathfindingParams.startingPosition.y, gridSize.x)];
        startNode.gCost = 0;
        ...
        else
        {
            ...
            pathIndexComponent.pathIndex = pathPositionBuffer.Length - 1;
        }
    ...
    /// <param name="pathPositionBuffer"> Store path in the IBufferElementData component. </param>
    private static void CalculatePath(NativeArray<PathNode> pathNodeArray, PathNode endNode, DynamicBuffer<pathfinding.PathPosition> pathPositionBuffer)
    {
        if (endNode.previousNodeIndex == -1)
        {
            // No path is found
        ...
        {
            // Found a path
            pathPositionBuffer.Add(new pathfinding.PathPosition { position = new int2(endNode.x, endNode.y) });

            PathNode currentNode = endNode;
            while (currentNode.previousNodeIndex != -1)
            {
                PathNode previousNode = pathNodeArray[currentNode.previousNodeIndex];
                pathPositionBuffer.Add(new pathfinding.PathPosition { position = new int2(previousNode.x, previousNode.y) });
